...
const icons = {
  ...
  gallery: "M3 4a2 2 0 012-2h14a2 2 0 012 2v16a2 2 0 01-2 2H5a2 2 0 01-2-2V4z M3 4l7.89 5.26a1.99 1.99 0 002.22 0L21 4",
    ...
    <div className="flex flex-col h-screen w-72 bg-slate-50 border-r border-slate-200 shadow-[2px_0_10px_rgba(0,0,0,0.02)]">
      ...
      <div className="p-6">
        <div className="flex items-center gap-3 px-2">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-800 tracking-tight leading-tight">K.H.S</h2>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 py-4 space-y-1">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <svg 
                className={`w-5 h-5 transition-colors ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-blue-600'}`}
                viewBox="0 0 24 24" 
                fill="none" 
                stroke="currentColor" 
                strokeWidth="2" 
      ...
      <div className="p-4 m-4 bg-slate-100/50 rounded-2xl border border-slate-200/60">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 overflow-hidden">
            ...
            <p className="text-[10px] text-slate-500 truncate">kigaliharvestschool@yahoo.fr</p>
          </div>
        </div>
      </div>
    </div>
